---
name: stylometry
description: Profile a writer's voice from a corpus and score drafts against an interpretable voice card.
agents: [codex, claude]
---
...
Two actions, both run via `tools/profiler.py`:
...
python tools/profiler.py build "posts/*.md" --out voice_card.json --label essays

# Score a draft against a voice card
python tools/profiler.py score path/to/draft.md --card voice_card.json

# Score from stdin
cat draft.md | python tools/profiler.py score - --card voice_card.json
```
...
- Zero-dependency Python; ships with this skill at `tools/profiler.py`.
